...
const GdprSection = ({ no, header, content }: { no?: number | undefined, header: string, content: string | ReactElement }) => (
  <>
    <h3 className="mt-6 font-medium">{no ? `${no}. ` : ''}{header}</h3>
...
const Gdpr = () => {
  ...
  return (
    <main className='max-w-kodekalender mx-auto mt-10'>
      <BackToDoorsButton />
      <DoorBorder />
      <div className='py-12 px-8 md:px-12 mx-4 md:mx-8 bg-gray-100 text-gray-900 rounded-md'>
          ...
          {sections.map(([header, content], i) => <GdprSection key={i} no={i + 1} header={header} content={content} />)}
